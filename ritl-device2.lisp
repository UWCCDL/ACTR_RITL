...
(defun save-log (&optional (filename "model.txt"))
  (let* ((log (mapcar #'summarize-trial
    ...
    (write-csv table filename)
	      (write-fmri-results (ensure-directories-exist
                           (make-pathname :name (format nil "model_act")  :type "txt"  )))
))
